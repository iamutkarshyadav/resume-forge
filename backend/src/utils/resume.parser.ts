...
function extractName(lines: string[]): string {
  ...
  for (let i = 0; i < Math.min(lines.length, 10); i++) {
    ...
    if (lower.includes("@") || /\d{3}/.test(lower)) continue;
    
    // Skip if it matches a common header exactly
    if (commonHeaders.has(lower.replace(/[:.]/g, ""))) continue;
    ...
    if (line.length > 2 && line.length < 50 && /^[a-zA-Z\s\-.']+$/.test(line)) {
        ...
        if (words.length > 4) continue; // Names rarely have > 4 parts
...
function sectionIndices(lines: string[]) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (sections.includes(line.replace(/[:]/g, ""))) {
      ...
      if (key.includes("work") || key.includes("professional")) key = "experience";
      indices[key] = i;
    }
  }
  return indices;
