...
function extractName(lines) {
    ...
    for (const line of lines) {
        ...
        if (!l)
            continue;
        if (/email|e-mail|phone|contact|curriculum vitae|resume/i.test(l))
            continue;
        if (/education|experience|projects|skills|summary|objective/i.test(l))
            continue;
        if (l.length > 3 && l.length < 80)
            return l.replace(/[^\p{L} .'-]/gu, "");
    }
    return "";
}
...
function toStructuredJSON(text) {
    ...
    const order = ["summary", "skills", "experience", "work experience", "projects", "education"];
    const sections = {};
