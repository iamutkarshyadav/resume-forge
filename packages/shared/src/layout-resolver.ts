import { ResumeAST, TemplateRules, ResumeSection } from "./ast";

export interface ResolvedLayout {
  header: ResumeAST["header"];
  sections: ResumeAST["sections"]; // Filtered and truncated
  meta: {
    droppedBullets: number;
    droppedEntries: number;
    pageCount: number; // Should be 1
  };
}
 ...
 */
export function resolveLayout(ast: ResumeAST, rules: TemplateRules): ResolvedLayout {
  ...
  for (const sectionId of rules.layout.sectionOrder) {
    ...
    // Deep copy to modify
    const section = JSON.parse(JSON.stringify(originalSection)) as ResumeSection;
    ...
    for (const entry of section.entries) {
        ...
        if (entry.company || entry.role) entryHeight += LINE_HEIGHT_PT;
